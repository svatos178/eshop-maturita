...
if (isset($_GET['hledat'])) {
    ...
    $query_hledani= ("SELECT * FROM produkt WHERE jmenoProdukt LIKE '%".$_GET['hledat']."%' OR
     kategorieProdukt LIKE '%".$_GET['hledat']."%' OR barvaProdukt LIKE '%".$_GET['hledat']."%' OR znackaProdukt LIKE '%".$_GET['hledat']."%' OR
      pohlaviProdukt LIKE '%".$_GET['hledat']."%' ");
 $stmt_hledani = $db->prepare($query_hledani);
   $stmt_hledani->execute();
   $result_hledani = $stmt_hledani->get_result()->fetch_all();
    ?>
...
<div class="row popis-vrsek">
                  ...
                  <li class="list-group-item filtr-width">
                    <div class=form-check>
                      <label class="from-check-label">
                        <input type="checkbox" class="form-check-input product-check" name="kategorieProdukt[]" value="<?= $result[0]; ?>"
                          <?php if (isset($_GET["kategorieProdukt"])){
              ...
              <ul class="list-group">
                ...
                ?>
                  <li class="list-group-item filtr-width">
                    <div class=form-check>
                      <label class="from-check-label">
                        <input type=checkbox class="form-check-input product-check" value="<?= $result[0]; ?>" id="znackaProdukt" name="znackaProdukt[]"
                        <?php if (isset($_GET["znackaProdukt"])){
              ...
              <ul class="list-group">
                ...
                ?>
                  <li class="list-group-item filtr-width">
                    <div class=form-check>
                      <label class="from-check-label">
                        <input type=checkbox class="form-check-input product-check" value="<?= $result[0]; ?>" id="barvaProdukt" name="barvaProdukt[]"
                        <?php if (isset($_GET["barvaProdukt"])){
              ...
              <ul class="list-group">
                ...
                ?>
                  <li class="list-group-item filtr-width">
                    <div class=form-check>
                      <label class="from-check-label">
                        <input type=checkbox class="form-check-input product-check" value="<?= $result[0]; ?>" id="pohlaviProdukt" name="pohlaviProdukt[]"
                        <?php if (isset($_GET["pohlaviProdukt"])){
                ...
                <div class="card filtr-width vyska-cena">
                  <div class="col-xs-4 form-group">
                    <label for="cenaProduktMin"> Minimální cena </label>
                      ...
                      $max = $result["maxCena"];
                      ?>
                    <input type="number" class="form-control" value="<?= $min ?>" id="cenaProduktMin" name="cenaProduktMin" 
                      min="<?= $min ?>" max="<?= $max ?>">
                  
                  </div>
                  <div class="form-group">
                    <label > Maximální cena </label>
                      <input type="number" class="form-control" id="cenaProduktMax" value="<?= $max ?>" name="cenaProduktMax" 
                      min="<?= $min ?>" max="<?= $max ?>">
    ...
    <?php  foreach ($result_hledani as $value) {
      ?>
      <div class="col-md-4 col-lg-3 d-flex pb-3" style="max-height: 600px;">
              <div class="card mb-4 lg-3 w-100">
          <form action="" method="post">
         <a href="produkt.php?id=<?php echo($value[0]) ?>">
          <img id="imgprodukt" src="<?php echo import_file($value[4]); ?>" alt="" srcset="" class="card-img-top card-img-sizing">
        </a>
            <div class="card-body ">
              <p class="card-text ">
              <a href="produkt.php?id=<?php echo($value[0]) ?>">  
               <h2 class="nazev-produkt"><?php echo $value[1]; ?></h2>
              </a>
            <?php  if (isset($_SESSION['idUzivatel']) && $email === "admin@email.cz") {
    ?>
              <h6 class="text-secondary">
                <?php
                echo "ID produktu: " . $value[0];
               ?> 
              </h6>
            <?php  } ?> 
              <div class=" overflow-auto " style="font-size: 13px; max-height: 90px; min-height: 90px; ">
               ...
               <h5 class="text-danger cena-polozky"><?php echo $value[3]; ?>,-</h5>   
                </p>
              </div>
            </form>
          </div>
